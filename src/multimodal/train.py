"""
Core training and evaluation logic for the multimodal fusion pipeline.
Called by train_fusion.py entry point.
"""
...
def evaluate(
    ...
    """Evaluate fusion model. Returns (avg_loss, metrics_dict)."""
    model.eval()
...
def train(model, train_loader, val_loader, cfg, device: torch.device):
    """
    Full training loop for the fusion model.
    ...
    Args:
        ...
        device: torch.device
    """
    logger = Logger(cfg.logging.log_dir, name="fusion", log_every_n_steps=cfg.logging.log_every_n_steps)
    ...
    # Use linear warmup scheduler when encoders are being fine-tuned
    freeze = cfg.multimodal.freeze_encoders
    scheduler = None
    if not freeze:
    ...
    for epoch in range(1, cfg.training.num_epochs + 1):
        ...
        if epoch % cfg.checkpoints.save_every_n_epochs == 0:
            save_checkpoint(model, optimizer, epoch, val_metrics, cfg.checkpoints.save_dir, name="fusion")

        if val_metrics["auroc"] > best_auroc:
            best_auroc = val_metrics["auroc"]
            save_checkpoint(model, optimizer, epoch, val_metrics, cfg.checkpoints.save_dir, name="fusion_best")
